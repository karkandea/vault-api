...
SET NOCOUNT ON;
                            ...
                            */
                            ...
                            WHERE pr.PR_Date >= '2024-01-01'
                                  AND CAST(tp.Order_Date AS DATE) >= @Order_Date_Begin
                                  AND CAST(tp.Order_Date AS DATE) <= @Order_Date_End;

                            DECLARE @EffectiveLength INT = CASE
                                                               WHEN @Length = 0
                                                                    OR @Length IS NULL THEN
                                                                   999999999
                                                               ELSE
                                                                   @Length
                                                           END;
...
-- ============ AGREGASI ORDER TYPE (LAMA af1e7763) ============
SELECT tp.Order_Type, COUNT(*) AS Jml
FROM #w_pr AS pr
    INNER JOIN #w_type_process AS tp
        ON tp._CategoryProcess_SubCategoryId = pr._CategoryProcess_SubCategoryId
           AND tp._TypeProcess_SubCategoryId = pr._TypeProcess_SubCategoryId
GROUP BY tp.Order_Type
ORDER BY tp.Order_Type;
