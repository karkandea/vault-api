-- ============ MEASURE A vs B (auto-generated) ============
SET NOCOUNT ON;
DECLARE @PR_No NVARCHAR(100) = NULL;
DECLARE @PR_Status_ValueId INT = NULL;
DECLARE @PR_Category_Id INT = NULL;
DECLARE @PR_Date_Begin DATE = NULL;
DECLARE @PR_Date_End DATE = NULL;
DECLARE @Department_Id INT = NULL;
DECLARE @Account_Code_Id INT = NULL;
DECLARE @Cost_Center_Id INT = NULL;
DECLARE @Vendor_Id INT = NULL;
DECLARE @Order_Type_Id INT = NULL;
DECLARE @Order_No NVARCHAR(100) = NULL;
DECLARE @Order_Status_ValueId INT = NULL;
DECLARE @Order_Date_Begin DATE = '2025-01-01';
DECLARE @Order_Date_End DATE = '2025-12-31';
DECLARE @Start INT = 0;
...
-- ============ A: TotalCount versi UI (tanpa dn/ipo) ============
SELECT COUNT(*) AS total_count_ui
FROM #w_pr AS pr
...
WHERE pr.PR_Date >= '2024-01-01'
      AND (@Order_Date_Begin IS NULL OR tp.Order_Date >= @Order_Date_Begin)
      AND (@Order_Date_End IS NULL OR tp.Order_Date <= @Order_Date_End);

-- ============ B: Data render (dengan LEFT JOIN dn + ipo) ============
SELECT
    COUNT(*) AS render_rows,
    COUNT(DISTINCT CONCAT(pr._Id, '|', tp._Id, '|', tp.Order_No)) AS render_distinct_po
FROM #w_pr AS pr
    INNER JOIN #w_type_process AS tp
        ON tp._CategoryProcess_SubCategoryId = pr._CategoryProcess_SubCategoryId
           AND tp._TypeProcess_SubCategoryId = pr._TypeProcess_SubCategoryId
