namespace APS_REST_API.Queries.Report
{
	...
	{
		public static string GetQueryReportPurchaseOrder(Models.Report.Procurement.DataTables.Request_WIP request)
		{
			string subQuery = @"AND pr._TypeProcess_SubCategoryId = @Order_Type_Id AND tp.Order_No like @Order_No AND pr._VendorId = @Vendor_Id";

			if (request.Order_Type_Id == null)
			{
				subQuery = subQuery.Replace("pr._TypeProcess_SubCategoryId = @Order_Type_Id", "1=1");
			}

			if (string.IsNullOrEmpty(request.Order_No))
			{
				subQuery = subQuery.Replace("tp.Order_No like @Order_No", "1=1");
			}

			if (request.Vendor_Id == null)
			{
				subQuery = subQuery.Replace("pr._VendorId = @Vendor_Id", "1=1");
			}
